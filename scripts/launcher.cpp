...
const char *TARGET_QQBOT_JS = "resources/app/loadQQBot.js";

// 注入的 JS 内容
const char *QQBOT_JS_CONTENT =
    "const path = require('path');"
    "const CurrentPath = process.env.QQBOT_BOOTMAIN || path.dirname(__filename);"
    "(async () => {"
    "    await import('file://' + path.join(CurrentPath, './qqbot/qqbot.mjs'));"
    "})();";
...
// 标记 loadQQBot.js 是否已生成
static bool g_loadqqbot_generated = false;
...
static int create_memfd_with_content(const char *content)
{
    ...
    int fd = syscall(SYS_memfd_create, "qqbot_memfd", 0);
    if (fd < 0)
        return -1;
...
/**
 * 启动时生成 loadQQBot.js 到当前目录
 */
__attribute__((constructor))
static void generate_loadqqbot_js()
{
    if (g_loadqqbot_generated)
        return;
    FILE *fp = fopen("loadQQBot.js", "w");
    if (fp)
    {
        fwrite(QQBOT_JS_CONTENT, 1, strlen(QQBOT_JS_CONTENT), fp);
        fclose(fp);
        g_loadqqbot_generated = true;
        printf("[launcher] loadQQBot.js generated in current directory\n");
    }
    else
    {
        printf("[launcher] failed to generate loadQQBot.js in current directory\n");
    }
}
...
static int handle_target_file(const char *pathname)
{
    ...
    else if (path_matches(pathname, TARGET_QQBOT_JS))
    {
        printf("[launcher] intercepted loadQQBot.js: %s, replacing content\n", pathname);
        printf("[launcher] loadQQBot.js content: %s\n", QQBOT_JS_CONTENT);
        return create_memfd_with_content(QQBOT_JS_CONTENT);
    }
